...
require("lazy").setup({
		...
		config = function()
			require("claude-code").setup({
				window = {
					position = "float",
					float = {
						width = "95%", -- Take up 90% of the editor width
						height = "97%", -- Take up 90% of the editor height
...

	{
		...
		keys = {
			...
			{
				"<leader>q", -- Reuse your old keybinding
				"<cmd>Trouble quickfix toggle<cr>",
				desc = "Quickfix List (Trouble)",
			},
			{
				"<leader>sd", -- Reuse your old telescope keybinding
				"<cmd>Trouble diagnostics toggle<cr>",
				desc = "[S]earch [D]iagnostics (Trouble)",
			},
		},
	},
	...
	},
	{
		...
		config = function()
			...
			local angularls_path = "$HOME/.nvm/versions/node/v18.19.1/lib/node_modules/@angular/language-server/"
			local angular_cmd = {
				"ngserver",
				"--stdio",
				"--tsProbeLocations",
		...
		config = function()
			...
			vim.api.nvim_create_autocmd("User", {
				...
				callback = function(args)
					vim.keymap.set("n", "<CR>", go_in_plus, { buffer = args.data.buf_id })
				end,
			})
